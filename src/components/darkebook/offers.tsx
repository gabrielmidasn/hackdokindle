        ...
        <ul className="text-left space-y-2 mb-6">
          {offer.features.map((feature: any, index: number) => (
            <li key={index} className="flex items-start">
              {feature.included ? <Check className="w-5 h-5 text-green-500 mr-2 shrink-0" /> : <X className="w-5 h-5 text-red-500 mr-2 shrink-0" />}
              <span>{feature.text}</span>
            </li>
          ))}
        </ul>
        <Button onClick={() => !isFeatured && onEssentialClick()} size="lg" className={`${isFeatured ? 'bg-gold text-charcoal-black' : 'bg-gray-600'} w-full font-bold text-lg`}>
          {offer.cta}
        </Button>
      </div>
    );
